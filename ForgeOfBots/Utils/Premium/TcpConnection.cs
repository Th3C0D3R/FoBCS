   ...
   public static class TcpConnection
   {
      ...
      public static object SendAuthData(string serial, string hwid, bool CheckOnly = false)
      {
         ...
         try
         {
            ...
            Thread thread = new Thread(o => ReceiveAuthData((TcpClient)o, serial, hwid, CheckOnly));
            thread.Start(client);
      ...
      private static void ReceiveAuthData(TcpClient client, string serial, string hwid, bool CheckOnly = false)
      {
      ...
      public static object SendScriptRequest(ScriptRequest eScriptRequest)
      {
         FirewallHelper.OpenFirewallPort(port, "ForgeOfBots");
         IPAddress ip = IPAddress.Parse(IP);
         TcpClient client = new TcpClient();
         try
         {
            client.Connect(ip, port);
            NetworkStream ns = client.GetStream();
            Thread thread = new Thread(o => ReceiveScript((TcpClient)o));
            thread.Start(client);
            byte[] buffer = Encoding.ASCII.GetBytes($"scriptType:{eScriptRequest}");
            ns.Write(buffer, 0, buffer.Length);
            thread.Join();
            while (!done)
               Application.DoEvents();
      ...
      private static void ReceiveScript(TcpClient client)
      {
         using (BinaryReader reader = new BinaryReader(client.GetStream()))
         {
            int length = reader.ReadInt32();
            ...
            if (dataRes == "xv2v3 3uv1y 004y8 9z5xz w3xuy xzvw3 z0".ToLower() || dataRes == "0yy92 y780v y3476 uz2v7 w9847 z45y6 uv".ToLower())
            {
               ...
               {
                  //Buffer == Script
                  var script = Encoding.ASCII.GetString(buffer);
               }
            }
            else if (dataRes == "4728u u2051 0u4y4 0z9yx 471vw 2531z y4".ToLower())
            {
